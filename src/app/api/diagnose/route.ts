...
${profileInfo}${postsText ? `\n\n${postsText}` : ''}${competitorInfo ? `\n\n${competitorInfo}` : ''}`;
    ...
    try {
      ...
      // すべてのリクエストに対して、標準語と鹿児島弁の両方を生成するよう指示を追加
      queryText += '\n\n【重要：出力形式の強制】\n以下の手順で2つのバージョンを作成し、指定の区切り文字で結合して出力せよ。\n\n1. 【標準語パート】\n   まずは「標準語（丁寧なビジネス口調）」で分析結果を出力する。\n   （ここには鹿児島弁を一切混ぜないこと）\n\n2. 【区切り】\n   次に、区切り文字 `<<<SEGODON_SPLIT>>>` を出力する。\n\n3. 【西郷どんパート】\n   最後に、上記と同じ分析内容を「西郷どん（鹿児島弁）」になりきって翻訳して出力する。\n   ・一人称：「おい」、二人称：「おはん」\n   ・語尾：「〜でごわす」「〜モス」\n   ・トーン：豪快かつ愛のある説教\n\n出力例:\n総合判定：B\n改善が必要です。\n<<<SEGODON_SPLIT>>>\n西郷どんの目利き：B\n気張らんといかんモス！\n\n【重要：出力スタイルの強制】\n標準語版と鹿児島弁版の両方を作成する際、分析内容は完全に同じにしてください。\n違いは言葉遣いのみです。鹿児島弁版では、一人称「おい」、二人称「おはん」、語尾「〜でごわす」「〜モス」「〜じゃっど」を使用し、豪快で熱血なトーンで出力してください。';
